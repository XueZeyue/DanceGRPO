...
def pipeline_with_logprob_transformer(
    self: StableDiffusion3Pipeline,
    prompt: Union[str, List[str]] = None,
    height: Optional[int] = None,
    width: Optional[int] = None,
    ...
    assert height % (self.vae_scale_factor * self.patch_size) == 0
    assert width % (self.vae_scale_factor * self.patch_size) == 0
    # 1. Check inputs. Raise error if not correct
    print(f"height: {height}, width: {width}")
    print(f"self.vae_scale_factor: {self.vae_scale_factor},self.patch_size: {self.patch_size}")
    negative_prompt_embeds = negative_prompt_embeds.squeeze(1)  # 从 [4, 1, 768] 变为 [4, 768]
    self.check_inputs(
        prompt,
        prompt_2=None,
        prompt_3=None,
        height=height,
        width=width,
        callback_on_step_end_tensor_inputs=callback,
